...
export default class booklistsearch extends Component {
  constructor(props) {
    super(props);

    this.srcvalue = ""
    this.arrival = ""
    this.state = {books: [],title: "All Books"};
  }
      ...
      .then(response => {
        ...
        this.setState({
          books: this.state.books.filter(
            book => (
              (book.BookName.indexOf((this.props.match.params.id))>= 0)||(book.Author.indexOf((this.props.match.params.id))>= 0)
            )
          )
        })
      })
  ...
  sortBook() {
    /* document,getBy */
    /* document.getElementById('titlesort').innerHTML += " (Sort By Highest Price)" */
    this.setState({title : "All Books (Sort By Highest Price)"})
    this.setState({
