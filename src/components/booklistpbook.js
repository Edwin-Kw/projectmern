...
export default class booklistpbook extends Component {
  constructor(props) {
    super(props);

    this.srcvalue = ""
    this.arrival = ""
    this.state = {books: [],title: "All Picture book"};
  }

  componentDidMount() {
    axios.get('http://localhost:5000/books/')
      .then(response => {
        this.setState({ books: response.data.filter(catebook => catebook.Category === 'Picture Book') })
  ...
  sortBook() {
    this.setState({title : "All Picture book (Sort By Highest Price)"})
    this.setState({
  ...
  render() {
    return (
      <div>
        <p><a href="/" onClick={() => { }}>Home </a> &gt; <a href="/booklistpbook" onClick={() => { }}>Picture book</a></p>
